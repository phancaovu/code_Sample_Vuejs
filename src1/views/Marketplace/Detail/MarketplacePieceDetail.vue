        ...
        <div class="mywallet-content">
          ...
          <div class="walltet-content-box p-4 d-flex align-items-start flex-column">
            <div class="content-box-top"><img class="w-100" src="../../../assets/images/eggs-box.png">
              <div class="content-box-top-eggs top-50 translate-middle-y text-center"><img class="w-50" :src="pieceImage(item.tokenId)"></div>
            </div>
            <div class="content-box-top w-100">
              <div class="content-box-top-name position-relative">
                <div class="d-flex justify-content-center align-items-center">
                  <div class="text-radiant text-uppercase text-center">{{ symbolSnake(item.tokenId) }}</div>
                  <div class="position-relative ms-2 snake-id"><img class="w-100" src="../../../assets/images/id-egg.png">
                    <div class="position-absolute top-50 start-50 translate-middle">#{{ item.tokenId }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="row mx-0">
          <div class="col-xxl-11 offset-xxl-1">
            <div class="d-md-flex align-items-center justify-content-center justify-content-lg-start">
              <h3 class="text-radiant text-uppercase text-center">{{ symbolSnake(item.tokenId) }}</h3>
              ...
              <div class="col-md-5 px-lg-0">
                <div class="d-grid">
                  <button class="btn btn-wallet py-3" type="button" @click="buyItem" :disabled="disable_buy">
                    <div class="text-radiant fs-5 fw-bold">{{ item.isSold ? 'Sold out' : 'Buy' }}</div>
                  </button>
                </div>
              </div>
            </div>
  ...
  methods: {
    pieceImage(tokenId) {
      ...
      let obj = _.find(snakes, snake => {
        return snake.id === Number(tokenId);
      });
      if (!obj)
        return null;
      let snake_type = obj.snake_type, skin_id = obj.skin_id;
    ...
    async approveContractNFT1155() {
      ...
      try {
        let contract = await vm.$store.dispatch('contract/getContractPiece', {
          contract_name: `SC_NEP_${vm.item.tokenId}`,
          signer: vm.account.wallet.signer
        });
        ...
        if (!isApproveAll) {
          vm.step_detail = {
            step: 1,
            message: 'Approve contract to buy NFT.'
          }
          let approve = await contract.setApprovalForAll(SC_MARKET_EGG_1155, true);
          vm.step_detail = {
            step: 1,
            message: 'Approve contract to buy NFT success, please wait...'
          }
          await approve.wait()
        }
        return true;
    ...
    'item': {
      ...
      async handler() {
        ...
        try {
          ...
          if (symbol.substr(0, 3) != 'NEP') {
            await vm.$router.push({name: 'MarketplacePiece'})
          }
        } catch (e) {
          await vm.$router.push({name: 'MarketplacePiece'})
        }
